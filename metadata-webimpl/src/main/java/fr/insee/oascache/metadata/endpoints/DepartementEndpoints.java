...
public class DepartementEndpoints implements GeoDepartementApi {
    ...
    @Override
    public ResponseEntity<Departement> getcogdep(String code, Optional<LocalDate> date) {
        return this.webResponseBuilder.forFindByCodeQuery(Departement.class, code, date).asOneObjet();
    }

    @Override
    public ResponseEntity<List<TerritoireRef>> getcogdepdesc(String code, Optional<LocalDate> date, Optional<String> filtreNom, Optional<TypeEnum> type) {
